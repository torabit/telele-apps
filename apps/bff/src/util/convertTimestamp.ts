...
/**
 *
 * @param timestamp googleのtimestamp
 * @returns GoogleTimestampをDateに変換します
 */
export function convertToDate(timestamp?: GoogleTimestamp): Date {
  if (!timestamp) {
    return null;
  }

  const googleTimestamp = new Timestamp();
  googleTimestamp.setSeconds(timestamp.seconds);
  googleTimestamp.setNanos(timestamp.nanos);

  const date = googleTimestamp.toDate();
  return date;
}
